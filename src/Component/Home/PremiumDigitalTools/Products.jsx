            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10'>
                {
                    products.map((product) => {
                        return (
                            <div key={product.id} className='border border-zinc-100 p-4 sm:p-5 rounded-2xl shadow-md hover:shadow-xl transition duration-300 space-y-4 relative bg-white'>
                                ...
                                <button className='btn bg-gradient-to-r from-[#4F39F6] to-[#9514FA] text-white rounded-full w-full py-2 sm:py-3 text-sm sm:text-base'>Buy Now</button>
                            </div>
                        )
                    })
                }
